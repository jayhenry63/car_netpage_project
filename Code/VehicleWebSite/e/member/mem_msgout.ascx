<% @  Control Language="c#" Inherits="PageAdmin.mem_msgout"%>
...
<form runat="server">
 <table border="0" cellspacing="0" cellpadding="0"  align="center" class="member_table_1"> 
    <tr>
      ...
      <td align=center  class="memlist_header_item" width="15%">发送方式</td> 
      <td align=center  class="memlist_header_item_last" width="15%">发送时间</td> 
    </tr>
<asp:Repeater id="P1" runat="server" OnItemDataBound="P1_Bound">      
 <ItemTemplate>
      <tr>
       <td align=left  class="memlist_item_1"><input type="checkbox" id="CK" Name="CK" value='<%#DataBinder.Eval(Container.DataItem,"id")%>'>&nbsp;<img src="/e/images/public/<%#Get_Sended(DataBinder.Eval(Container.DataItem,"sended").ToString())%>" border=0> <a href='<%#Get_Url("mem_msgoutdtl",DataBinder.Eval(Container.DataItem,"id").ToString())%>' ><%#Server.HtmlEncode(DataBinder.Eval(Container.DataItem,"title").ToString())%></a></td>  
       <td align=center  class="memlist_item_1"><%#Get_SendType(DataBinder.Eval(Container.DataItem,"sendtype").ToString())%></td> 
 ...
 { 
  ...
  if(IDS=="0")
  ...
  else
   {
    if(confirm("确定删除吗?"))
     {
      location.href="?act=delete&ids="+IDS+"&type=mem_msgout&s=<%=SiteId%>";
     }
   }
 }
