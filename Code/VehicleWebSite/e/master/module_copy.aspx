...
<script language="c#" runat="server">
 ...
 string CurrentSiteId,CurrentLanmuId,UserName,SiteList,LanmuList,SubLanmuList,Admin_Sites,Admin_Lanmuids,Sql_Admin_Lanmuids,List_Space,currentstyle,sql;
 int UID,List_Level;
...
private void Data_Copy()
 {
    ...
    conn.Open();
    OleDbCommand comm;
    string Fields="name,thetable,sort_id,model_id,show_type,notshow_type,zdy_sort,zdy_condition,title_num,titlepic_width,titlepic_height,show_num,starthtml,endhtml,iswrap,tourl,title_show,zdy_style,zdy_header,layout,width,height,target,show,zdy_tag_open,zdy_tag,content,zt_id,xuhao";
    for(int i=0;i<Aids.Length;i++)
     {
       sql="insert into pa_module(lanmu_id,sublanmu_id,"+Fields+")  select  "+LanmuId+","+SubLanmuId+","+Fields+" from pa_module where id="+int.Parse(Aids[i]);
       comm=new OleDbCommand(sql,conn);
...
<table border=0 cellpadding=5 cellspacing=0 width=100% align=center>
 ...
 <tr>
  ...
  <td><select name="lanmuid" id="lanmuid" onchange="c_lanmu(this.value)">
...
function c_site(siteid)
 {
   location.href="?c_lanmuid=<%=Request.QueryString["c_lanmuid"]%>&c_sublanmuid=<%=Request.QueryString["c_sublanmuid"]%>&siteid="+siteid;
 }
function c_lanmu(lanmuid)
 {
   location.href="?c_lanmuid=<%=Request.QueryString["c_lanmuid"]%>&c_sublanmuid=<%=Request.QueryString["c_sublanmuid"]%>&siteid=<%=CurrentSiteId%>&lanmuid="+lanmuid;
 }
