...
private void Add_Order(string Table,int DetailId,string Title,int Number,double MPrice,int SendPoint)
 {
   ...
   if(!dr.Read())
    {
     sql="insert into pa_orderlist(username,anonymous,thetable,detail_id,title,num,member_price,sendpoint,[color],[size],[type]) values('"+UserName+"','"+Anonymous+"','"+Table+"',"+DetailId+",'"+Title+"',"+Number+","+MPrice+","+SendPoint+",'"+color+"','"+size+"','"+type+"')";
