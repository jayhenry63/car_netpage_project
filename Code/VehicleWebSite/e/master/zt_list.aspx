      ...
      <tr>
        <td  align="left">
          ...
          <asp:Repeater id="P1" runat="server" OnItemDataBound="Data_Bound">         
             <ItemTemplate>
                 <tr class="listitem">
                  <td align=left class="tdstyle" noWrap><input type="checkbox" id="CK" Name="CK" Value="<%#DataBinder.Eval(Container.DataItem,"id")%>"><a href="<%#Get_Url(DataBinder.Eval(Container.DataItem,"id").ToString(),DataBinder.Eval(Container.DataItem,"thetype").ToString(),DataBinder.Eval(Container.DataItem,"lanmu_dir").ToString(),DataBinder.Eval(Container.DataItem,"zdy_url").ToString())%>" target=_blank><%#Server.HtmlEncode(DataBinder.Eval(Container.DataItem,"name").ToString())%></a></td>
                  ...
                  <td align=center class="tdstyle" noWrap>
                   ...
                   <asp:Hyperlink id="Lanmu_Set"  runat="server"  NavigateUrl='<%#"javascript:General_Set(\"专题设置\",\"lanmu_set.aspx?iszt=1&id="+DataBinder.Eval(Container.DataItem,"id")+"&lanmu="+DataBinder.Eval(Container.DataItem,"name").ToString()+"\")"%>'  Text="专题设置" />
